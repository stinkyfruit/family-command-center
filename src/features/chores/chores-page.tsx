...
function ChildIncentiveProgress({ child, chores, mode, targetCents, targetStars, earnedCentsByMember, paidOutCentsByMember }: { child: Member; chores: ChoreEntry[]; mode: ChoreRewardMode; targetCents: number; targetStars: number; earnedCentsByMember: Record<string, number>; paidOutCentsByMember: Record<string, number> }) {
  ...
  const balanceClass = availableCents < 0 ? (isRainbowCard ? "text-rose-700" : "text-rose-700 dark:text-rose-200") : (isRainbowCard ? "text-emerald-700" : "text-emerald-700 dark:text-emerald-200");
  const balanceValue = availableCents < 0 ? `-$${Math.abs(availableCents / 100).toFixed(2)}` : `$${(availableCents / 100).toFixed(2)}`;
  ...
  if (mode === "money" && availableCents < 0) {
    return <article className={`relative overflow-hidden rounded-2xl p-4 ${progressCardClass}`}><div className="flex items-start justify-between gap-2"><div><p className={`text-[10px] font-black uppercase tracking-[0.16em] ${balanceClass}`}>Borrowed</p><p className={`mt-0.5 text-3xl font-black leading-none ${balanceClass}`}>{balanceValue}</p><p className={`mt-1 text-xs font-bold ${secondaryTextClass}`}>Earned: ${(runningEarnedCents / 100).toFixed(2)} · Paid out: ${(paidOutCents / 100).toFixed(2)}</p></div><span className={`rounded-full bg-rose-100 px-2.5 py-1 text-xs font-black text-rose-700 ${isRainbowCard ? "" : "dark:bg-rose-400/20 dark:text-rose-200"}`}>{isWeekend ? "Weekend" : "Today"}: $${(dailyEarned / 100).toFixed(2)} / $${(target / 100).toFixed(2)}</span></div><p className={`mt-3 text-sm font-bold ${secondaryTextClass}`}>Complete chores to get back to $0.00, then start earning again.</p><div className={`mt-2 h-3 overflow-hidden rounded-full ${progressTrackClass}`}><div className="h-full rounded-full bg-gradient-to-r from-rose-400 via-amber-400 to-emerald-500 transition-[width] motion-reduce:transition-none" style={{ width: `${progress}%` }} /></div></article>;
  }
