...
export function WeatherCard({ weather, sunTimes, auroraActivity, cometCloseApproach, onOpenForecast }: WeatherCardProps) {
  ...
  return <article className={`weather-card relative w-full overflow-hidden rounded-[1.75rem] p-5 text-white shadow-lg max-md:p-5 md:max-lg:p-6 md:p-6 ${isNightWeather ? "weather-card-night shadow-indigo-950/40" : "weather-card-day shadow-sky-200/50"}`}><span className={`weather-card-orb absolute -right-5 -top-9 size-28 rounded-full transition-colors duration-500 max-md:-top-6 max-md:size-20 ${weatherOrbClass(weather)}`} /><div className="weather-card-content relative min-w-0"><div className="flex h-full min-w-0 items-center justify-between gap-3 max-md:gap-2 md:gap-6 md:max-lg:gap-4"><div className="min-w-0 flex-1"><p title={`${timeGreeting()} · ${weather?.location ?? "LOCAL FORECAST"}`} className="truncate text-xs font-bold leading-tight tracking-wide md:max-lg:text-sm md:text-sm">{timeGreeting()} · {weather?.location ?? "LOCAL FORECAST"}</p>{isStorm && <span className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-violet-950/30 px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-white ring-1 ring-white/25"><AppIcon name="storm" className="size-3.5" />Active storm</span>}<p className="mt-2 text-4xl font-black tracking-tighter max-md:text-3xl md:max-lg:text-5xl md:text-5xl">{weather ? `${weather.temperature}°` : "—"}</p><p className="min-h-10 line-clamp-2 text-sm font-semibold leading-snug text-white/90 md:min-h-0 md:max-lg:text-base md:text-base">{weather ? `${weather.summary} · ↑ ${weather.high}° ↓ ${weather.low}°` : "Allow location for today’s weather"}</p>{windLabel && <p className="mt-1 truncate text-xs font-bold text-white/80">{windLabel}{weather?.conditionSource === "observation" ? " · Live observation" : ""}</p>}{isNightWeather && weather && <MoonPhaseBadge phase={currentMoonPhase}/>}</div><span className="block size-24 shrink-0 overflow-hidden max-md:size-28 md:size-40 md:max-lg:size-28">{weather ? <WeatherAnimation weather={weather} isNight={isNightWeather} /> : <span className="block text-6xl leading-none drop-shadow-sm md:text-8xl">{isNightWeather ? "🌙" : "☀️"}</span>}</span></div><WeatherSkyDetails sunTimes={sunTimes} notableSkyEvent={notableSkyEvent} auroraActivity={auroraActivity} /><button type="button" onClick={onOpenForecast} className="mt-4 inline-flex items-center gap-1.5 rounded-lg px-1 py-1 text-xs font-black text-white/90 transition hover:bg-white/10 hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white disabled:cursor-not-allowed disabled:opacity-50" disabled={!weather}><span>{weather ? "Forecast & outdoor conditions" : "Forecast unavailable"}</span><AppIcon name="chevronRight" className="size-3.5" /></button></div></article>;
}
