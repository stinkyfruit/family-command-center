  ...
  return <section className="mx-auto max-w-[1800px] px-5 pb-24 md:px-9 lg:pb-8">
    <div className="mx-auto max-w-6xl space-y-6">
      <SettingsGroup id="settings-personal" eyebrow="PERSONAL" title="Your settings" description="Set the way this family home feels for you.">
        <article className="rounded-2xl bg-sky-50 p-5 dark:bg-sky-400/10"><p className="font-bold">Appearance</p><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Auto follows local sunrise and sunset using the weather location.</p><div className="mt-4 flex flex-wrap gap-2">{([ ["auto", "◐ Auto"], ["light", "☀ Light"], ["dark", "☾ Dark"] ] as const).map(([mode, label]) => <button key={mode} type="button" onClick={() => onThemeModeChange(mode)} className={`rounded-xl px-4 py-2 text-sm font-bold ${themeMode === mode ? "bg-sky-600 text-white shadow-sm" : "bg-white text-sky-800 ring-1 ring-sky-200 hover:bg-sky-100 dark:bg-white/10 dark:text-sky-100 dark:ring-white/10"}`}>{label}</button>)}</div></article>
...
function SettingsGroup({ id, eyebrow, title, description, children }: { id: string; eyebrow: string; title: string; description: string; children: ReactNode }) {
  return <section id={id} className="scroll-mt-32 rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-slate-100 dark:bg-white/5 dark:ring-white/10 md:p-6"><div className="border-b border-slate-100 pb-5 dark:border-white/10"><p className="text-sm font-black uppercase tracking-wide text-indigo-600 dark:text-indigo-300">{eyebrow}</p><h2 className="mt-1 text-2xl font-black text-slate-900 dark:text-white">{title}</h2><p className="mt-1 text-sm font-medium text-slate-500 dark:text-slate-300">{description}</p></div><div className="mt-5 space-y-5">{children}</div></section>;
}
