...
function ChoreRewardSettings({ members, chores, mode, targetCents, targetStars, onEditReward }: { members: Member[]; chores: ChoreEntry[]; mode: ChoreRewardMode; targetCents: number; targetStars: number; onEditReward: (chore: ChoreEntry) => void }) {
  ...
  return <section className="mx-auto w-full min-w-0 max-w-[1800px] overflow-hidden px-5 pb-5 md:px-9"><div className="w-full min-w-0 max-w-full overflow-hidden rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-slate-100 dark:bg-white/5 dark:ring-white/10"><div className="flex min-w-0 flex-wrap items-end justify-between gap-3"><div className="min-w-0"><p className="text-sm font-black uppercase tracking-wide text-violet-600">DAILY TEMPLATE</p><h2 className="mt-1 text-2xl font-black text-slate-900 dark:text-white">Routine reward values</h2><p className="mt-1 text-sm font-medium text-slate-500 dark:text-slate-300">Set these once. The same morning and nighttime values repeat every day.</p></div><span className="shrink-0 rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-black text-emerald-700 dark:bg-emerald-400/15 dark:text-emerald-200">{mode === "money" ? "Cents" : "Stars"}</span></div><div className="mt-4 min-w-0 space-y-4">{children.map((child) => { const childChores = routineChores.filter((chore) => chore.assigneeMemberId === child.id); const dailyCents = childChores.reduce((sum, chore) => sum + chore.rewardCents, 0); const dailyStars = childChores.reduce((sum, chore) => sum + chore.rewardStars, 0); const total = mode === "money" ? dailyCents : dailyStars; const difference = target - total; return <div key={child.id} className="min-w-0 max-w-full overflow-hidden rounded-2xl bg-slate-50 p-3 dark:bg-white/5"><div className="flex min-w-0 flex-wrap items-center justify-between gap-2"><h3 className="min-w-0 truncate font-black text-slate-800 dark:text-slate-100">{child.name}</h3><div className="min-w-0 text-right"><p className={`truncate text-xs font-black ${difference === 0 ? "text-emerald-600 dark:text-emerald-300" : "text-amber-600 dark:text-amber-300"}`}>{mode === "money" ? `Daily total: $${(dailyCents / 100).toFixed(2)} / $${(targetCents / 100).toFixed(2)}` : `Daily total: ${dailyStars} / ${targetStars} stars`}</p>{difference !== 0 && <p className="truncate text-[10px] font-bold text-slate-500 dark:text-slate-300">{difference > 0 ? `${mode === "money" ? `$${(difference / 100).toFixed(2)}` : difference} remaining` : `${mode === "money" ? `$${(Math.abs(difference) / 100).toFixed(2)}` : Math.abs(difference)} over target`}</p>}</div></div><div className="mt-2 grid min-w-0 gap-2 sm:grid-cols-2 lg:grid-cols-3">{childChores.map((chore) => <div key={chore.id} className="flex min-w-0 max-w-full items-center gap-2 overflow-hidden rounded-xl bg-white px-3 py-2 dark:bg-white/5"><span className="min-w-0 flex-1 truncate text-sm font-bold text-slate-700 dark:text-slate-100" title={chore.title}>{chore.title}</span><span className="shrink-0 text-sm font-black text-emerald-600 dark:text-emerald-300">{formatReward(chore.rewardCents, chore.rewardStars, mode)}</span><button type="button" onClick={() => onEditReward(chore)} aria-label={`Edit reward for ${chore.title}`} className="grid size-8 shrink-0 place-items-center rounded-lg text-violet-600 hover:bg-violet-100 dark:text-violet-300 dark:hover:bg-white/10"><AppIcon name="edit" className="size-4" /></button></div>)}</div></div>; })}</div></div></section>;
}
