...
export type SettingsSectionId = "settings-personal" | "settings-notifications" | "settings-family" | "settings-calendars" | "settings-chores" | "settings-account";
...
const settingsSections: Array<{ id: SettingsSectionId; label: string; description: string; icon: IconName }> = [
  ...
  { id: "settings-notifications", label: "Notifications", description: "Phone alerts", icon: "bell" },
...
export function SettingsNavigation({ showChores, showAccount }: { showChores: boolean; showAccount: boolean }) {
  ...
  const renderedActiveSection = visibleSections.some((section) => section.id === activeSection) ? activeSection : visibleSections[0]?.id ?? "settings-personal";
  ...
  return <>
    <section className="mx-auto w-full min-w-0 max-w-6xl px-5 md:px-9" aria-labelledby="settings-page-title">
    ...
    <div className="sticky top-3 z-30 mx-auto mt-3 w-full min-w-0 max-w-6xl px-5 md:px-9">
      <div className="rounded-[1.5rem] bg-indigo-50/95 p-2 shadow-lg shadow-indigo-950/10 ring-1 ring-indigo-100 backdrop-blur dark:bg-slate-900/95 dark:ring-white/10">
        <nav aria-label="Settings sections" className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
          {visibleSections.map((section) => <a key={section.id} href={`#${section.id}`} aria-controls={section.id} aria-current={renderedActiveSection === section.id ? "location" : undefined} className={`group flex min-w-0 items-center gap-2 rounded-xl px-3 py-2.5 text-left ring-1 transition hover:-translate-y-0.5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 ${renderedActiveSection === section.id ? "bg-indigo-600 text-white shadow-sm ring-indigo-600 dark:bg-indigo-500 dark:ring-indigo-500" : "bg-white/85 ring-indigo-100 hover:bg-white dark:bg-white/10 dark:ring-white/10 dark:hover:bg-white/15"}`}>
            <span className={`grid size-8 shrink-0 place-items-center rounded-lg ${renderedActiveSection === section.id ? "bg-white/15 text-white" : "bg-indigo-100 text-indigo-700 dark:bg-indigo-300/15 dark:text-indigo-200"}`}><AppIcon name={section.icon} className="size-4" /></span>
            <span className="min-w-0"><span className={`block truncate text-xs font-black sm:text-sm ${renderedActiveSection === section.id ? "text-white" : "text-slate-800 dark:text-white"}`}>{section.label}</span><span className={`hidden truncate text-[11px] font-semibold sm:block ${renderedActiveSection === section.id ? "text-white/75" : "text-slate-500 dark:text-slate-300"}`}>{section.description}</span></span>
            <AppIcon name="chevronRight" className={`ml-auto size-4 shrink-0 transition-transform group-hover:translate-x-0.5 ${renderedActiveSection === section.id ? "text-white/80" : "text-indigo-500"}`} />
          </a>)}
        </nav>
      </div>
    </div>
