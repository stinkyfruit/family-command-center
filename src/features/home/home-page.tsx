...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function loadMembership() {
      ...
      setHouseholdId(id);
      if (id) {
        const { data: household } = await supabase!.from("households").select("name, theme_mode, show_chores_tab, show_wishlist_tab, show_movie_night_tab, chore_reward_mode, chore_reward_target_cents, chore_reward_target_stars").eq("id", id).single();
        if (household) {
  ...
  const visibleNavigationTabs = navigationTabs.filter(([tab]) => tab !== "chores" || showChoresTab).filter(([tab]) => tab !== "wishlist" || showWishlistTab).filter(([tab]) => tab !== "movie-night" || showMovieNightTab);
