    ...
    <article className={`rounded-[2rem] p-6 shadow-sm ring-1 ring-white/70 dark:ring-white/10 ${colors[colorIndex % colors.length]}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h2 className="text-xl font-bold"><button type="button" onClick={onToggleExpanded} aria-expanded={expanded} aria-controls={contentId} className="flex max-w-full items-center gap-2 text-left"><span className="truncate">{list.title}</span><AppIcon name={expanded ? "chevronDown" : "chevronRight"} className="size-4 shrink-0" /></button></h2>
        </div>
