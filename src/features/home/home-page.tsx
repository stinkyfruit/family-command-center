...
export default function Home() {
  ...
  return (
    <main inert={celebrationActive || undefined} className={`${dark ? "dark " : ""}h-dvh overflow-x-hidden overflow-y-auto overscroll-y-auto`}>
      ...
      {weekendChoreDraft && <WeekendChoreEditor draft={weekendChoreDraft} mode={choreRewardMode} memberName={members.find((member) => String(member.id) === weekendChoreDraft.memberId)?.name ?? "Child"} onClose={() => setWeekendChoreDraft(null)} onSave={async (draft) => { await addChore(draft.memberId, "Weekend", draft.title, new Date().toLocaleDateString("en-CA"), draft.reward); setWeekendChoreDraft(null); }} />}
...
function TemporaryRoutineChores({ members, chores, mode, onAddChore, onEditReward, onDeleteChore }: { members: Member[]; chores: ChoreEntry[]; mode: ChoreRewardMode; onAddChore: (memberId: string | number, routine: string) => void; onEditReward: (chore: ChoreEntry) => void; onDeleteChore: (chore: ChoreEntry) => void }) {
...
function SettingsPage({ choreRewardMode, choreRewardTargetCents, choreRewardTargetStars, earnedCentsByMember, paidOutCentsByMember, onPayOut, onResetToday, onClearAll, onAddChore, onDeleteChore, onRewardModeChange, chores, onEditReward, ...props }: SettingsPageProps) {
  ...
  return <><SettingsPageContent {...props} onUnlocked={() => setSettingsUnlocked(true)} />{settingsUnlocked && <><TabVisibilitySettings showChoresTab={props.showChoresTab} showWishlistTab={props.showWishlistTab} onTabVisibilityChange={props.onTabVisibilityChange} />{props.showChoresTab && <ChoreSettingsSection choreRewardMode={choreRewardMode} choreRewardTargetCents={choreRewardTargetCents} choreRewardTargetStars={choreRewardTargetStars} earnedCentsByMember={earnedCentsByMember} paidOutCentsByMember={paidOutCentsByMember} onPayOut={onPayOut} onResetToday={onResetToday} onClearAll={onClearAll} onAddChore={onAddChore} onDeleteChore={onDeleteChore} onRewardModeChange={onRewardModeChange} chores={chores} onEditReward={onEditReward} members={props.members} currentMember={currentMember} />}</>}{supabase && <section className="mx-auto max-w-[1800px] px-5 pb-24 md:px-9 lg:pb-8"><div className="max-w-2xl rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-100 dark:bg-white/5 dark:ring-white/10"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm font-bold text-violet-600">ACCOUNT</p><h2 className="mt-1 text-2xl font-bold">Sign out</h2><p className="mt-1 text-sm text-slate-500 dark:text-slate-300">Sign out of this family command center on this device.</p></div><button type="button" onClick={() => void handleSignOut()} className="flex items-center gap-2 rounded-xl bg-rose-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-rose-700"><AppIcon name="signOut" className="size-4" />Log out</button></div>{signOutMessage && <p className="mt-3 text-sm font-semibold text-rose-700 dark:text-rose-200">{signOutMessage}</p>}</div></section>}</>;
}

function ChoreSettingsSection({ choreRewardMode, choreRewardTargetCents, choreRewardTargetStars, earnedCentsByMember, paidOutCentsByMember, onPayOut, onResetToday, onClearAll, onAddChore, onDeleteChore, onRewardModeChange, chores, onEditReward, members, currentMember }: { choreRewardMode: ChoreRewardMode; choreRewardTargetCents: number; choreRewardTargetStars: number; earnedCentsByMember: Record<string, number>; paidOutCentsByMember: Record<string, number>; onPayOut: (childMemberId: string | number, amountCents: number) => Promise<{ error?: string }>; onResetToday: () => Promise<{ error?: string; deleted?: number }>; onClearAll: () => Promise<{ error?: string; deleted?: number }>; onAddChore: (memberId: string | number, routine: string) => void; onDeleteChore: (chore: ChoreEntry) => void; onRewardModeChange: (mode: ChoreRewardMode) => void; chores: ChoreEntry[]; onEditReward: (chore: ChoreEntry) => void; members: Member[]; currentMember?: Member }) {
  return <section className="mx-auto max-w-[1800px] px-5 pb-5 md:px-9"><div className="rounded-[2rem] bg-amber-50 p-5 shadow-sm ring-1 ring-amber-100 dark:bg-amber-400/10 dark:ring-amber-300/20"><p className="text-sm font-black uppercase tracking-wide text-amber-700 dark:text-amber-200">CHORE SETTINGS</p><h2 className="mt-1 text-2xl font-black">Chores</h2><p className="mt-1 text-sm font-medium text-slate-600 dark:text-slate-300">Set the daily rewards, add weekend chores, and manage children&apos;s earnings in one place.</p><div className="mt-5 space-y-1"><p className="px-1 text-xs font-black uppercase tracking-wide text-slate-500 dark:text-slate-300">Rewards</p><ChoreRewardModeSettings mode={choreRewardMode} onModeChange={onRewardModeChange} /><ChoreRewardSettings members={members} chores={chores} mode={choreRewardMode} targetCents={choreRewardTargetCents} targetStars={choreRewardTargetStars} onEditReward={onEditReward} /><p className="px-1 pt-3 text-xs font-black uppercase tracking-wide text-slate-500 dark:text-slate-300">Weekend chores</p><WeekendChoresSettings members={members} chores={chores} mode={choreRewardMode} onAddChore={onAddChore} onDeleteChore={onDeleteChore} />{currentMember?.role === "adult" && <><p className="px-1 pt-3 text-xs font-black uppercase tracking-wide text-slate-500 dark:text-slate-300">Earnings &amp; cleanup</p><ChorePayoutSettings members={members} earnedCentsByMember={earnedCentsByMember} paidOutCentsByMember={paidOutCentsByMember} onPayOut={onPayOut} /><ChoreResetSettings onResetToday={onResetToday} onClearAll={onClearAll} /></>}</div></div></section>;
}
...
function SettingsPageContent({ members, currentUserId, onMemberColorChange, onAddMember, onRemoveMember, onUpdateCurrentMemberName, themeMode, onThemeModeChange, onUnlocked, googleConnections, appleFeeds, onConnect, onToggleConnection, onAddApple, onToggleApple, onInviteAdult, onSignOut }: { members: Member[]; currentUserId: string | null; onMemberColorChange: (memberId: string | number, color: string) => Promise<void>; onAddMember: (name: string, role: Member["role"]) => Promise<{ error?: string }>; onRemoveMember: (member: Member) => Promise<{ error?: string }>; onUpdateCurrentMemberName: (name: string) => Promise<{ error?: string }>; themeMode: ThemeMode; onThemeModeChange: (mode: ThemeMode) => void; showChoresTab: boolean; showWishlistTab: boolean; onTabVisibilityChange: (tab: "chores" | "wishlist", visible: boolean) => void; onUnlocked?: () => void; googleConnections: GoogleConnection[]; appleFeeds: AppleFeed[]; onConnect: () => void; onToggleConnection: (connection: GoogleConnection) => void; onAddApple: (name: string, url: string) => void; onToggleApple: (feed: AppleFeed) => void; onInviteAdult: (email: string, displayName: string) => Promise<{ link?: string; error?: string }>; onSignOut: () => Promise<{ error?: string }> }) {
  ...
  if (!currentMember) return <section className="mx-auto max-w-[1800px] px-5 pb-24 md:px-9 lg:pb-8"><div className="max-w-2xl rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-100 dark:bg-white/5 dark:ring-white/10"><p className="text-sm font-bold text-violet-600">SETTINGS</p><p className="mt-2 text-sm text-slate-500 dark:text-slate-300">Your family profile is still loading.</p></div></section>;
