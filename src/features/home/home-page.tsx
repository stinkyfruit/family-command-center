...
export default function Home() {
  ...
  async function saveTodo(event: FormEvent) {
    ...
    const currentMemberId = members.find((member) => user?.id === member.userId)?.id;
    ...
    if (editingTodo && supabase && householdId) {
      ...
      if (changedToAnotherMember) void requestNtfyNotification({ event: "task_assigned", householdId, targetMemberId: String(assigneeMemberId), taskTitle: title, dueDate: todoDueDate || null });
    } else if (editingTodo) {
      setTodos((items) => items.map((todo) => todo.id === editingTodo.id ? { ...todo, title, due: dueAt ? new Date(dueAt).toLocaleDateString([], { weekday: "short" }) : "", dueAt, assigneeMemberId } : todo));
      ...
      if (data && changedToAnotherMember) void requestNtfyNotification({ event: "task_assigned", householdId, targetMemberId: String(assigneeMemberId), taskTitle: title, dueDate: todoDueDate || null });
    } else {
  ...
  async function saveMoodCheckin() {
    ...
    if (supabase && householdId && user) {
      ...
      if (data && moodChanged && member.role === "child") void requestNtfyNotification({ event: "child_mood_changed", householdId, childMemberId: String(member.id), mood: selectedMood });
    }
