...
import { eventOccursOn as calendarEventOccursOn, isBirthdayEvent, shiftCalendar } from "@/components/home/calendar";
import {
...
import { SettingsPageContent as GroupedSettingsPageContent, type SettingsPageContentProps } from "@/features/settings/settings-content";
...
function SettingsPage({ choreRewardMode, choreRewardTargetCents, choreRewardTargetStars, earnedCentsByMember, paidOutCentsByMember, onPayOut, onResetToday, onClearAll, onAddChore, onDeleteChore, onRewardModeChange, chores, onEditReward, ...props }: SettingsPageProps) {
  ...
  return <>
    ...
    <GroupedSettingsPageContent {...props} onUnlocked={() => setSettingsUnlocked(true)} />
    {settingsUnlocked && <>
...
function ChoreSettingsSection({ activeTab, onTabChange, choreRewardMode, choreRewardTargetCents, choreRewardTargetStars, earnedCentsByMember, paidOutCentsByMember, onPayOut, onResetToday, onClearAll, onAddChore, onDeleteChore, onRewardModeChange, chores, onEditReward, members, currentMember }: { activeTab: ChoreSettingsTab; onTabChange: (tab: ChoreSettingsTab) => void; choreRewardMode: ChoreRewardMode; choreRewardTargetCents: number; choreRewardTargetStars: number; earnedCentsByMember: Record<string, number>; paidOutCentsByMember: Record<string, number>; onPayOut: (childMemberId: string | number, amountCents: number) => Promise<{ error?: string }>; onResetToday: () => Promise<{ error?: string; deleted?: number }>; onClearAll: () => Promise<{ error?: string; deleted?: number }>; onAddChore: (memberId: string | number, routine: string) => void; onDeleteChore: (chore: ChoreEntry) => void; onRewardModeChange: (mode: ChoreRewardMode) => void; chores: ChoreEntry[]; onEditReward: (chore: ChoreEntry) => void; members: Member[]; currentMember?: Member }) {
  ...
  return <section id="settings-chores" className="scroll-mt-32 mx-auto max-w-[1800px] px-5 pb-5 md:px-9"><div className="rounded-[2rem] bg-amber-50 p-5 shadow-sm ring-1 ring-amber-100 dark:bg-amber-400/10 dark:ring-amber-300/20"><p className="text-sm font-black uppercase tracking-wide text-amber-700 dark:text-amber-200">CHORE SETTINGS</p><h2 className="mt-1 text-2xl font-black">Chores</h2><p className="mt-1 text-sm font-medium text-slate-600 dark:text-slate-300">Choose a chore setting to expand it.</p><div className="mt-5 grid gap-5 md:grid-cols-[14rem_minmax(0,1fr)]"><nav aria-label="Chore settings" className="flex gap-2 overflow-x-auto md:block md:space-y-2">{tabs.map(([tab, label, description]) => <button key={tab} type="button" onClick={() => onTabChange(tab)} aria-current={activeTab === tab ? "page" : undefined} className={`w-52 shrink-0 rounded-2xl p-3 text-left transition md:w-full ${activeTab === tab ? "bg-amber-500 text-white shadow-md" : "bg-white/70 text-slate-700 hover:bg-white dark:bg-white/10 dark:text-slate-100"}`}><span className="block text-sm font-black">{label}</span><span className={`mt-1 block text-xs font-semibold ${activeTab === tab ? "text-white/80" : "text-slate-500 dark:text-slate-300"}`}>{description}</span></button>)}</nav><div className="min-w-0">{activeTab === "rewards" && <><ChoreRewardModeSettings mode={choreRewardMode} onModeChange={onRewardModeChange} /><ChoreRewardSettings members={members} chores={chores} mode={choreRewardMode} targetCents={choreRewardTargetCents} targetStars={choreRewardTargetStars} onEditReward={onEditReward} /></>}{activeTab === "weekend" && <WeekendChoresSettings members={members} chores={chores} mode={choreRewardMode} onAddChore={onAddChore} onDeleteChore={onDeleteChore} />}{activeTab === "earnings" && currentMember?.role === "adult" && <><ChorePayoutSettings members={members} earnedCentsByMember={earnedCentsByMember} paidOutCentsByMember={paidOutCentsByMember} onPayOut={onPayOut} /><ChoreResetSettings onResetToday={onResetToday} onClearAll={onClearAll} /></>}{activeTab === "earnings" && currentMember?.role !== "adult" && <p className="rounded-2xl bg-white/70 p-5 text-sm font-semibold text-slate-600 dark:bg-white/10 dark:text-slate-300">Earnings and testing controls are available to adults only.</p>}</div></div></div></section>;
}
