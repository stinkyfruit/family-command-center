...
function draftFromDish(dish: DinnerDish): DishDraft {
  return { id: dish.id, title: dish.title, category: dish.category, photoFile: null, photoPath: dish.photoPath, photoUrl: dish.photoUrl, photoRemoved: false, ratings: Object.fromEntries(dish.ratings.map((rating) => [String(rating.memberId), String(rating.rating)])) };
}
...
function DinnerPhoto({ url, title, className = "h-44 w-full" }: { url: string | null; title: string; className?: string }) {
...
export function FamilyDinnersPage({ householdId, members, currentUserId, sharedLists, onAddListItem, onToggleListItem, onDeleteListItem, onOpenSharedLists }: FamilyDinnersPageProps) {
  ...
  useEffect(() => {
    ...
    async function loadDinners() {
      ...
      const dishResult = await supabase.from("family_dinner_dishes").select("id, dinner_id, position, title, category, photo_path").in("dinner_id", dinnerIds);
      if (dishResult.error) { setMessage(`Could not load dinner dishes: ${dishResult.error.message}`); setLoading(false); return; }
      const dishRows = (dishResult.data ?? []) as Array<{ id: string; dinner_id: string; position: number; title: string; category: string; photo_path: string | null }>;
      ...
      const loaded = dinnerRows.map((dinner) => ({
        ...
        notes: dinner.notes,
        dishes: dishRows.filter((dish) => dish.dinner_id === dinner.id).sort((first, second) => first.position - second.position).map((dish) => ({ id: dish.id, position: dish.position, title: dish.title, category: categories.some((option) => option.value === dish.category) ? dish.category as DinnerCategory : "main", photoPath: dish.photo_path, photoUrl: photoUrlByDish.get(dish.id) ?? null, ratings: ratingRows.filter((rating) => rating.dish_id === dish.id).map((rating) => ({ id: rating.id, memberId: rating.member_id, memberName: rating.member_name, rating: rating.rating })) })),
      }));
  ...
  function beginEditingDinner(dinner: Dinner) {
    ...
    setDishDrafts(dinner.dishes.map(draftFromDish));
  ...
  async function saveDinner(event: FormEvent<HTMLFormElement>) {
    ...
    const preparedDishes = dishDrafts.map((dish, index) => ({ ...dish, position: index + 1, title: dish.title.trim(), ratings: members.map((member) => ({ memberId: member.id, memberName: member.name, rating: Number(dish.ratings[String(member.id)] ?? "") })) }));
    ...
    try {
      if (!supabase || !householdId) {
        const localDinner: Dinner = { id: editingDinnerId ?? `local-${Date.now()}`, eatenOn: eatenOn || null, createdAt: existingDinner?.createdAt ?? new Date().toISOString(), notes: notes.trim() || null, dishes: preparedDishes.map((dish) => ({ id: dish.id ?? `local-dish-${Date.now()}-${dish.position}`, position: dish.position, title: dish.title, category: dish.category, photoPath: null, photoUrl: dish.photoUrl, ratings: dish.ratings.map((rating, ratingIndex) => ({ id: `local-rating-${Date.now()}-${ratingIndex}`, memberId: String(rating.memberId), memberName: rating.memberName, rating: rating.rating })) })) };
        ...
        notify(editingDinnerId ? "Family dinner updated." : "Family dinner saved.", "success");
        resetForm();
        return;
      }
      ...
      const persistedDishes: Array<{ id: string; position: number; title: string; category: DinnerCategory; photo_path: string | null }> = [];
      for (const dish of preparedDishes) {
        const dishData = { dinner_id: dinner.id, position: dish.position, title: dish.title, category: dish.category, photo_path: dish.photoRemoved ? null : dish.photoPath };
        const dishResult = dish.id
          ? await supabase.from("family_dinner_dishes").update(dishData).eq("id", dish.id).eq("dinner_id", dinner.id).select("id, position, title, category, photo_path").single()
          : await supabase.from("family_dinner_dishes").insert(dishData).select("id, position, title, category, photo_path").single();
        ...
        if (dish.photoFile) {
          ...
          const photoUpdate = await supabase.from("family_dinner_dishes").update({ photo_path: path }).eq("id", persisted.id).select("id, position, title, category, photo_path").single();
      ...
      const photoUrlByDish = new Map(photoUrls);
      const savedDinner: Dinner = { id: dinner.id, eatenOn: dinner.eaten_on, createdAt: dinner.created_at, notes: dinner.notes, dishes: persistedDishes.map((dish) => ({ id: dish.id, position: dish.position, title: dish.title, category: dish.category, photoPath: dish.photo_path, photoUrl: photoUrlByDish.get(dish.id) ?? null, ratings: (ratingResult.data ?? []).filter((rating) => rating.dish_id === dish.id).map((rating) => ({ id: rating.id, memberId: rating.member_id, memberName: rating.member_name, rating: rating.rating })) })) };
  ...
  async function deleteDinner(dinner: Dinner) {
    if (!await confirm(`Delete the family dinner from ${formatDate(dinner.eatenOn)}?`, { title: "Delete family dinner?", destructive: true })) return;
    setDeletingId(dinner.id);
    if (supabase && householdId && !dinner.id.startsWith("local-")) {
      const paths = dinner.dishes.map((dish) => dish.photoPath).filter((path): path is string => Boolean(path));
      if (paths.length) await supabase.storage.from(PHOTO_BUCKET).remove(paths);
      const result = await supabase.from("family_dinners").delete().eq("id", dinner.id).eq("household_id", householdId);
      if (result.error) { notify(`Could not delete this family dinner: ${result.error.message}`); setDeletingId(null); return; }
    }
    setDinners((current) => current.filter((item) => item.id !== dinner.id));
    setSelectedDinner(null);
    setDeletingId(null);
    notify("Family dinner deleted.", "success");
  }
  ...
  return <section className="mx-auto max-w-[1800px] px-5 pb-24 md:px-9 lg:pb-8">
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      {!showForm && (loading ? <div role="status" className="rounded-[2rem] bg-white p-10 text-center shadow-sm ring-1 ring-slate-100 dark:bg-white/5 dark:ring-white/10"><p className="font-black">Loading family dinners…</p><p className="mt-1 text-sm font-semibold text-slate-400">Gathering your favorite meals.</p></div> : message ? <div role="alert" className="rounded-2xl bg-rose-50 p-5 text-sm font-bold text-rose-800 ring-1 ring-rose-100 dark:bg-rose-400/10 dark:text-rose-200 dark:ring-rose-300/20">{message}</div> : dinners.length === 0 ? <div className="rounded-[2rem] border-2 border-dashed border-orange-200 bg-white/70 px-5 py-14 text-center dark:border-orange-300/20 dark:bg-white/5"><div className="mx-auto grid size-16 place-items-center rounded-3xl bg-orange-100 text-orange-700 dark:bg-orange-400/20 dark:text-orange-200"><AppIcon name="familyDinners" className="size-8" /></div><h2 className="mt-5 text-xl font-black">Your dinner history starts here</h2><p className="mx-auto mt-2 max-w-md text-sm font-medium text-slate-500 dark:text-slate-300">Record a family meal to remember what you made, who loved it, and what you&apos;d make again.</p><button type="button" onClick={() => setShowForm(true)} className="mt-5 rounded-xl bg-orange-600 px-4 py-2.5 text-sm font-black text-white hover:bg-orange-700">Record the first dinner</button></div> : <section aria-label="Recorded family dinners" className="grid gap-5 md:grid-cols-2">{dinners.map((dinner) => <button type="button" key={dinner.id} onClick={() => setSelectedDinner(dinner)} className="group overflow-hidden rounded-[2rem] bg-white text-left shadow-sm ring-1 ring-slate-100 transition hover:-translate-y-1 hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 dark:bg-white/5 dark:ring-white/10"><div className="bg-gradient-to-br from-orange-100 via-amber-50 to-yellow-50 p-5 dark:from-orange-400/20 dark:via-amber-400/10 dark:to-yellow-400/10"><div className="flex items-start justify-between gap-3"><span className="rounded-full bg-white/80 px-3 py-1 text-xs font-black text-orange-800 shadow-sm dark:bg-white/10 dark:text-orange-100">{shortDate(dinner.eatenOn)}</span><AppIcon name="familyDinners" className="size-6 text-orange-500 transition-transform group-hover:rotate-6 dark:text-orange-200" /></div><div className="mt-5 grid grid-cols-2 gap-3">{dinner.dishes.slice(0, 4).map((dish) => <DinnerPhoto key={dish.id} url={dish.photoUrl} title={dish.title} />)}</div></div><div className="p-5"><h2 className="text-xl font-black">{dinner.dishes.map((dish) => dish.title).join(" · ")}</h2><p className="mt-2 text-sm font-semibold text-slate-500 dark:text-slate-300">{dinner.dishes.length} {dinner.dishes.length === 1 ? "dish" : "dishes"}{dinner.notes ? " · Notes included" : ""}</p><div className="mt-4 flex flex-wrap gap-2">{dinner.dishes.slice(0, 3).map((dish) => <span key={dish.id} className="rounded-full bg-orange-100 px-2.5 py-1 text-xs font-black text-orange-800 dark:bg-orange-400/20 dark:text-orange-100">{dish.title}: {averageRating(dish).toFixed(1)} / 10</span>)}</div></div></button>)}</section>)}
    </div>

    {selectedDinner && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedDinner(null); }}><article role="dialog" aria-modal="true" aria-labelledby="selected-dinner-title" className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] bg-white p-5 shadow-2xl dark:bg-[#202031] md:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-orange-600 dark:text-orange-300">Family dinner</p><h2 id="selected-dinner-title" className="mt-1 text-2xl font-black">{formatDate(selectedDinner.eatenOn)}</h2></div><button type="button" onClick={() => setSelectedDinner(null)} aria-label="Close dinner details" className="grid size-9 place-items-center rounded-xl text-slate-500 hover:bg-slate-100 dark:hover:bg-white/10"><AppIcon name="close" className="size-5" /></button></div>{selectedDinner.notes && <div className="mt-6 rounded-2xl bg-amber-50 p-4 dark:bg-amber-400/10"><p className="text-xs font-black uppercase tracking-wide text-amber-800 dark:text-amber-100">Dinner notes</p><p className="mt-2 whitespace-pre-wrap text-sm font-semibold leading-relaxed text-slate-700 dark:text-slate-200">{selectedDinner.notes}</p></div>}<div className="mt-6 grid gap-4">{selectedDinner.dishes.map((dish) => <section key={dish.id} className="rounded-2xl bg-orange-50 p-4 dark:bg-orange-400/10"><div className="grid gap-4 sm:grid-cols-[9rem_1fr]"><DinnerPhoto url={dish.photoUrl} title={dish.title} className="h-36 w-full sm:h-36" /><div><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-xs font-black uppercase tracking-wide text-orange-600 dark:text-orange-200">{categoryLabel(dish.category)}</p><h3 className="mt-1 text-xl font-black">{dish.title}</h3></div><span className="rounded-full bg-amber-100 px-3 py-1 text-sm font-black text-amber-800 dark:bg-amber-400/20 dark:text-amber-100">Family average {averageRating(dish).toFixed(1)} / 10</span></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{dish.ratings.map((rating) => <div key={rating.id} className="flex items-center justify-between rounded-xl bg-white/80 px-3 py-2 text-sm font-bold dark:bg-white/10"><span>{rating.memberName}</span><span className="text-orange-700 dark:text-orange-200">{rating.rating} / 10</span></div>)}</div></div></div></section>)}</div><div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => beginEditingDinner(selectedDinner)} className="inline-flex items-center gap-2 rounded-xl bg-orange-600 px-3 py-2 text-sm font-bold text-white hover:bg-orange-700"><AppIcon name="edit" className="size-4" />Edit</button><button type="button" onClick={() => void deleteDinner(selectedDinner)} disabled={deletingId === selectedDinner.id} className="inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-rose-600 hover:bg-rose-50 disabled:opacity-60 dark:hover:bg-rose-400/10"><AppIcon name="trash" className="size-4" />{deletingId === selectedDinner.id ? "Deleting…" : "Delete record"}</button></div></article></div>}
  </section>;
}
