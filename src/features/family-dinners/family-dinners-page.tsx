...
export function FamilyDinnersPage({ householdId, members, currentUserId, sharedLists, onAddListItem, onToggleListItem, onDeleteListItem, onOpenSharedLists }: FamilyDinnersPageProps) {
  ...
  async function saveDinner(event: FormEvent<HTMLFormElement>) {
    ...
    const preparedDishes = dishDrafts.map((dish, index) => ({ ...dish, position: index + 1, title: dish.title.trim(), madeByMemberId: dish.madeByMemberId || null, madeByMemberName: members.find((member) => String(member.id) === dish.madeByMemberId)?.name ?? null, ratings: members.map((member) => ({ memberId: member.id, memberName: member.name, rating: Number(dish.ratings[String(member.id)] ?? "") })) }));
  ...
  return <section className="mx-auto max-w-[1800px] px-5 pb-24 md:px-9 lg:pb-8">
    ...
    {selectedDinner && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedDinner(null); }}><article role="dialog" aria-modal="true" aria-labelledby="selected-dinner-title" className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] bg-white p-5 shadow-2xl dark:bg-[#202031] md:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-orange-600 dark:text-orange-300">Family dinner</p><h2 id="selected-dinner-title" className="mt-1 text-2xl font-black">{formatDate(selectedDinner.eatenOn)}</h2></div><button type="button" onClick={() => setSelectedDinner(null)} aria-label="Close dinner details" className="grid size-9 place-items-center rounded-xl text-slate-500 hover:bg-slate-100 dark:hover:bg-white/10"><AppIcon name="close" className="size-5" /></button></div>{selectedDinner.notes && <div className="mt-6 rounded-2xl bg-amber-50 p-4 dark:bg-amber-400/10"><p className="text-xs font-black uppercase tracking-wide text-amber-800 dark:text-amber-100">Dinner notes</p><p className="mt-2 whitespace-pre-wrap text-sm font-semibold leading-relaxed text-slate-700 dark:text-slate-200">{selectedDinner.notes}</p></div>}<div className="mt-6 grid gap-4">{selectedDinner.dishes.map((dish) => <section key={dish.id} className="rounded-2xl bg-orange-50 p-4 dark:bg-orange-400/10"><div className="grid gap-4 sm:grid-cols-[9rem_1fr]"><DinnerPhoto url={dish.photoUrl} title={dish.title} className="h-36 w-full sm:h-36" /><div><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-xs font-black uppercase tracking-wide text-orange-600 dark:text-orange-200">{categoryLabel(dish.category)}</p><h3 className="mt-1 text-xl font-black">{dish.title}</h3>{dish.madeByMemberName && <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-300">Made by {dish.madeByMemberName}</p>}</div><span className="rounded-full bg-amber-100 px-3 py-1 text-sm font-black text-amber-800 dark:bg-amber-400/20 dark:text-amber-100">Family average {averageRating(dish).toFixed(1)} / 10</span></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{dish.ratings.map((rating) => <div key={rating.id} className="flex items-center justify-between rounded-xl bg-white/80 px-3 py-2 text-sm font-bold dark:bg-white/10"><span>{rating.memberName}</span><span className="text-orange-700 dark:text-orange-200">{rating.rating} / 10</span></div>)}</div></div></div></section>)}</div><div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => beginEditingDinner(selectedDinner)} className="inline-flex items-center gap-2 rounded-xl bg-orange-600 px-3 py-2 text-sm font-bold text-white hover:bg-orange-700"><AppIcon name="edit" className="size-4" />Edit</button><button type="button" onClick={() => void deleteDinner(selectedDinner)} disabled={deletingId === selectedDinner.id} className="inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-rose-600 hover:bg-rose-50 disabled:opacity-60 dark:hover:bg-rose-400/10"><AppIcon name="trash" className="size-4" />{deletingId === selectedDinner.id ? "Deleting…" : "Delete record"}</button></div></article></div>}
  </section>;
}
