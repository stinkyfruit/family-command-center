...
type DinnerMealType = (typeof mealTypes)[number]["value"];
...
type Dinner = { id: string; eatenOn: string | null; createdAt: string; mealType: DinnerMealType; notes: string | null; dishes: DinnerDish[] };
...
function mealTypeLabel(mealType: DinnerMealType) {
  return mealTypes.find((option) => option.value === mealType)?.label ?? "Homemade";
}

function normalizeMealType(value: string | null | undefined): DinnerMealType {
  return value === "takeout" ? "takeout" : "homemade";
}

function mealTypeClassName(mealType: DinnerMealType) {
  return mealType === "takeout"
    ? "rounded-full bg-sky-100 px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-sky-800 dark:bg-sky-400/20 dark:text-sky-100"
    : "rounded-full bg-emerald-100 px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-emerald-800 dark:bg-emerald-400/20 dark:text-emerald-100";
}
...
function topRatedDishesForFilter(dinners: Dinner[], ratingFilter: "all" | DinnerCategory, mealTypeFilter: "all" | DinnerMealType) {
  const grouped = new Map<string, RatedDish>();
  for (const dinner of dinners) {
    if (mealTypeFilter !== "all" && dinner.mealType !== mealTypeFilter) continue;
...
export const FamilyDinnersPage = memo(function FamilyDinnersPage({ householdId, members, currentUserId, sharedLists, onAddListItem, onToggleListItem, onDeleteListItem, onOpenSharedLists }: FamilyDinnersPageProps) {
  ...
  const [mealType, setMealType] = useState<DinnerMealType>("homemade");
  ...
  const [mealTypeFilter, setMealTypeFilter] = useState<"all" | DinnerMealType>("all");
  ...
  useEffect(() => {
    ...
    async function loadDinners() {
      ...
      const dinnerResult = await supabase.from("family_dinners").select("id, eaten_on, meal_type, notes, created_at").eq("household_id", householdId).order("eaten_on", { ascending: false, nullsFirst: false }).order("created_at", { ascending: false });
      if (cancelled) return;
      ...
      const dinnerRows = (dinnerResult.data ?? []) as Array<{ id: string; eaten_on: string | null; meal_type: string | null; notes: string | null; created_at: string }>;
      ...
      const loaded = dinnerRows.map((dinner) => ({
        ...
        mealType: normalizeMealType(dinner.meal_type),
        notes: dinner.notes,
  ...
  const topRatedDishes = useMemo(() => topRatedDishesForFilter(dinners, ratingFilter, mealTypeFilter), [dinners, mealTypeFilter, ratingFilter]);
  ...
  async function saveDinner(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!supabase || !householdId) {
        const localDinner: Dinner = { id: editingDinnerId ?? `local-${Date.now()}`, eatenOn: eatenOn || null, createdAt: existingDinner?.createdAt ?? new Date().toISOString(), mealType, notes: notes.trim() || null, dishes: preparedDishes.map((dish) => ({ id: dish.id ?? `local-dish-${Date.now()}-${dish.position}`, position: dish.position, title: dish.title, category: dish.category, madeByMemberId: dish.madeByMemberId, madeByMemberName: dish.madeByMemberName, photoPath: null, photoUrl: dish.photoUrl, ratings: dish.ratings.map((rating, ratingIndex) => ({ id: `local-rating-${Date.now()}-${ratingIndex}`, memberId: String(rating.memberId), memberName: rating.memberName, rating: rating.rating })) })) };
      ...
      const dinnerResult = editingDinnerId
        ? await supabase.from("family_dinners").update({ eaten_on: eatenOn || null, meal_type: mealType, notes: notes.trim() || null }).eq("id", editingDinnerId).eq("household_id", householdId).select("id, eaten_on, meal_type, notes, created_at").single()
        : await supabase.from("family_dinners").insert({ household_id: householdId, eaten_on: eatenOn || null, meal_type: mealType, notes: notes.trim() || null, created_by: currentUserId }).select("id, eaten_on, meal_type, notes, created_at").single();
      if (dinnerResult.error || !dinnerResult.data) throw new Error(dinnerResult.error?.message ?? "The family dinner could not be saved.");
      const dinner = dinnerResult.data as { id: string; eaten_on: string | null; meal_type: string | null; notes: string | null; created_at: string };
      ...
      const photoUrlByDish = new Map(photoUrls);
      const savedDinner: Dinner = { id: dinner.id, eatenOn: dinner.eaten_on, createdAt: dinner.created_at, mealType: normalizeMealType(dinner.meal_type), notes: dinner.notes, dishes: persistedDishes.map((dish) => ({ id: dish.id, position: dish.position, title: dish.title, category: dish.category, madeByMemberId: dish.made_by_member_id, madeByMemberName: dish.made_by_member_name, photoPath: dish.photo_path, photoUrl: photoUrlByDish.get(dish.id) ?? null, ratings: (ratingResult.data ?? []).filter((rating) => rating.dish_id === dish.id).map((rating) => ({ id: rating.id, memberId: rating.member_id, memberName: rating.member_name, rating: rating.rating })) })) };
    ...
    <div className="mx-auto w-full min-w-0 max-w-6xl space-y-5">
      ...
      <div className="grid min-w-0 gap-5 lg:grid-cols-[1.2fr_.8fr]">
        <section className="min-w-0 rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-orange-100 dark:bg-white/5 dark:ring-white/10 md:p-6" aria-labelledby="top-rated-dishes-title">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-orange-600 dark:text-orange-300">Family favorites</p><h2 id="top-rated-dishes-title" className="mt-1 text-2xl font-black">Highest rated dishes</h2><p className="mt-1 text-sm font-medium text-slate-500 dark:text-slate-300">The dishes your family has loved most so far.</p></div><div className="flex flex-wrap gap-2"><StyledSelect value={ratingFilter} onChange={(event) => setRatingFilter(event.target.value as "all" | DinnerCategory)} className="mt-0 w-40" aria-label="Filter highest rated dishes by category"><option value="all">All types</option>{categories.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</StyledSelect><StyledSelect value={mealTypeFilter} onChange={(event) => setMealTypeFilter(event.target.value as DinnerMealType)} className="mt-0 w-36" aria-label="Filter highest rated dishes by meal type"><option value="all">All meals</option>{mealTypes.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</StyledSelect></div></div>
