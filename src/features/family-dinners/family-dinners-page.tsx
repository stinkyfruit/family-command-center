...
function DinnerPhoto({ url, title, className = "h-44 w-full", fit = "cover" }: { url: string | null; title: string; className?: string; fit?: "cover" | "contain" }) {
  return <div className={`grid place-items-center overflow-hidden rounded-2xl bg-orange-100 text-orange-600 dark:bg-orange-400/20 dark:text-orange-200 ${className}`}>{url ? <img src={url} alt={`${title} photo`} className={`size-full ${fit === "contain" ? "object-contain" : "object-cover"}`} loading="lazy" /> : <AppIcon name="familyDinners" className="size-10" />}</div>;
}
