...
function DinnerPhoto({ url, title, className = "h-44 w-full", fit = "cover" }: { url: string | null; title: string; className?: string; fit?: "cover" | "contain" | "natural" }) {
  return <div className={`grid place-items-center overflow-hidden rounded-2xl bg-orange-100 text-orange-600 dark:bg-orange-400/20 dark:text-orange-200 ${className}`}>{url ? <img src={url} alt={`${title} photo`} className={fit === "natural" ? "block max-h-80 max-w-[92%] object-contain" : fit === "contain" ? "size-full object-contain" : "size-full object-cover"} loading="lazy" /> : <AppIcon name="familyDinners" className="size-10" />}</div>;
}
