...
type MovieNight = { id: string; watchedOn: string; location: MovieNightLocation; createdAt: string; notes: string | null; movies: MovieNightMovie[] };
type MovieDraft = { id: string | null; title: string; pickerId: string; tmdbId: number | null; posterPath: string | null; releaseYear: number | null; ratings: Record<string, string>; firstTimeWatching: Record<string, boolean>; searchQuery: string; searchResults: TmdbSearchResult[]; searching: boolean; searchMessage: string };

function blankMovie(): MovieDraft {
  return { id: null, title: "", pickerId: "", tmdbId: null, posterPath: null, releaseYear: null, ratings: {}, firstTimeWatching: {}, searchQuery: "", searchResults: [], searching: false, searchMessage: "" };
}
...
export function MovieNightPage({ householdId, members, currentUserId }: { householdId: string | null; members: Member[]; currentUserId: string | null }) {
  ...
  const [location, setLocation] = useState<MovieNightLocation>("downstairs");
  ...
  useEffect(() => {
    ...
    async function loadMovieNights() {
      ...
      const nightResult = await supabase.from("movie_nights").select("id, watched_on, location, notes, created_at").eq("household_id", householdId).order("watched_on", { ascending: false }).order("created_at", { ascending: false });
      if (cancelled) return;
      ...
      const loadedNights = (nightResult.data ?? []) as Array<{ id: string; watched_on: string; location: string | null; notes: string | null; created_at: string }>;
  ...
  async function saveMovieNight(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const nightResult = editingNightId
        ? await supabase.from("movie_nights").update({ watched_on: watchedOn, location, notes: notes.trim() || null }).eq("id", editingNightId).eq("household_id", householdId).select("id, watched_on, location, notes, created_at").single()
        : await supabase.from("movie_nights").insert({ household_id: householdId, watched_on: watchedOn, location, notes: notes.trim() || null, created_by: currentUserId }).select("id, watched_on, location, notes, created_at").single();
      if (nightResult.error || !nightResult.data) throw new Error(nightResult.error?.message ?? "The movie night could not be saved.");
      const night = nightResult.data as { id: string; watched_on: string; location: string | null; notes: string | null; created_at: string };
