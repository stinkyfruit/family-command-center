...
type MovieDraft = { title: string; pickerId: string; tmdbId: number | null; posterPath: string | null; releaseYear: number | null; ratings: Record<string, string>; searchQuery: string; searchResults: TmdbSearchResult[]; searching: boolean; searchMessage: string };

function blankMovie(): MovieDraft {
  return { title: "", pickerId: "", tmdbId: null, posterPath: null, releaseYear: null, ratings: {}, searchQuery: "", searchResults: [], searching: false, searchMessage: "" };
}
    ...
    try {
      if (!supabase || !householdId) {
        const localNight: MovieNight = {
          ...
          movies: preparedMovies.map((movie) => ({ id: `local-${Date.now()}-${movie.position}`, position: movie.position, title: movie.title, pickerMemberId: movie.pickerId ? String(movie.pickerId) : null, pickerName: movie.pickerName, tmdbId: movie.tmdbId, posterPath: movie.posterPath, releaseYear: movie.releaseYear, ratings: movie.ratings.map((rating, index) => ({ id: `local-rating-${index}`, memberId: String(rating.memberId), memberName: rating.memberName, rating: rating.rating })) })),
        };
      ...
      const nightResult = await supabase.from("movie_nights").insert({ household_id: householdId, watched_on: watchedOn, notes: notes.trim() || null, created_by: currentUserId }).select("id, watched_on, notes, created_at").single();
      ...
      const movieResult = await supabase.from("movie_night_movies").insert(preparedMovies.map((movie) => ({ movie_night_id: night.id, position: movie.position, title: movie.title, picker_member_id: movie.pickerId, picker_name: movie.pickerName, tmdb_id: movie.tmdbId, poster_path: movie.posterPath, release_year: movie.releaseYear }))).select("id, position, title, picker_member_id, picker_name, tmdb_id, poster_path, release_year");
      if (movieResult.error || !movieResult.data) throw new Error(movieResult.error?.message ?? "The movies could not be created.");
      const insertedMovies = movieResult.data as Array<{ id: string; position: number; title: string; picker_member_id: string | null; picker_name: string | null; tmdb_id: number | null; poster_path: string | null; release_year: number | null }>;
