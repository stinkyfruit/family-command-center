...
export const MovieNightPage = memo(function MovieNightPage({ householdId, members, currentUserId }: { householdId: string | null; members: Member[]; currentUserId: string | null }) {
  ...
  async function deleteMovieNight(night: MovieNight) {
    if (!await confirm(`Permanently delete the movie night from ${formatDate(night.watchedOn)}?`, { title: "Delete movie night?", destructive: true })) return;
    ...
    try {
      if (supabase && householdId && !night.id.startsWith("local-")) {
        ...
        if (error) { notify(`Could not delete this movie night: ${error.message}`); return; }
      }
      setNights((current) => current.filter((item) => item.id !== night.id));
      if (selectedNight?.id === night.id) setSelectedNight(null);
