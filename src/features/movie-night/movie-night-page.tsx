...
export function MovieNightPage({ householdId, members, currentUserId }: { householdId: string | null; members: Member[]; currentUserId: string | null }) {
  ...
  useEffect(() => {
    ...
    async function loadMovieNights() {
      ...
      if (!cancelled) {
        setNights(sortMovieNights(loadedNights.map((night) => movieNightFromRows(night, loadedMovies.filter((movie) => movie.movie_night_id === night.id), (ratingResult.data ?? []) as Array<{ id: string; movie_id: string; member_id: string | null; member_name: string; rating: number }>))));
        setLoading(false);
      }
    }
