...
type MovieNightMovie = { id: string; position: number; title: string; pickerMemberId: string | null; pickerName: string | null; tmdbId: number | null; posterPath: string | null; releaseYear: number | null; ratings: MovieNightRating[] };
type MovieNight = { id: string; watchedOn: string; createdAt: string; notes: string | null; movies: MovieNightMovie[] };
type MovieDraft = { id: string | null; title: string; pickerId: string; tmdbId: number | null; posterPath: string | null; releaseYear: number | null; ratings: Record<string, string>; searchQuery: string; searchResults: TmdbSearchResult[]; searching: boolean; searchMessage: string };

function blankMovie(): MovieDraft {
  return { id: null, title: "", pickerId: "", tmdbId: null, posterPath: null, releaseYear: null, ratings: {}, searchQuery: "", searchResults: [], searching: false, searchMessage: "" };
}
...
function movieNightFromRows(night: { id: string; watched_on: string; notes: string | null; created_at: string }, movieRows: Array<{ id: string; position: number; title: string; picker_member_id: string | null; picker_name: string | null; tmdb_id: number | null; poster_path: string | null; release_year: number | null }>, ratingRows: Array<{ id: string; movie_id: string; member_id: string | null; member_name: string; rating: number }>): MovieNight {
  return {
    id: night.id,
...
export function MovieNightPage({ householdId, members, currentUserId }: { householdId: string | null; members: Member[]; currentUserId: string | null }) {
  ...
  useEffect(() => {
    ...
    async function loadMovieNights() {
      ...
      const movieResult = await supabase.from("movie_night_movies").select("id, movie_night_id, position, title, picker_member_id, picker_name, tmdb_id, poster_path, release_year").in("movie_night_id", nightIds);
      if (movieResult.error) {
        setMessage(`Could not load the movies: ${movieResult.error.message}`);
        setLoading(false);
        return;
      }
      const loadedMovies = (movieResult.data ?? []) as Array<{ id: string; movie_night_id: string; position: number; title: string; picker_member_id: string | null; picker_name: string | null; tmdb_id: number | null; poster_path: string | null; release_year: number | null }>;
  ...
  async function searchMovies(movieIndex: number) {
    const query = (movieDrafts[movieIndex]?.searchQuery || movieDrafts[movieIndex]?.title || "").trim();
    if (query.length < 2) {
  ...
  function selectMovie(movieIndex: number, result: TmdbSearchResult) {
    updateMovie(movieIndex, { title: result.title, tmdbId: result.id, posterPath: result.posterPath, releaseYear: result.releaseYear, searchQuery: result.title, searchResults: [], searchMessage: "" });
  }
    ...
    try {
      if (!supabase || !householdId) {
        const localNight: MovieNight = {
          ...
          movies: preparedMovies.map((movie) => ({ id: movie.id ?? `local-${Date.now()}-${movie.position}`, position: movie.position, title: movie.title, pickerMemberId: movie.pickerId ? String(movie.pickerId) : null, pickerName: movie.pickerName, tmdbId: movie.tmdbId, posterPath: movie.posterPath, releaseYear: movie.releaseYear, ratings: movie.ratings.map((rating, index) => ({ id: `local-rating-${Date.now()}-${index}`, memberId: String(rating.memberId), memberName: rating.memberName, rating: rating.rating })) })),
        };
        setNights((current) => editingNightId ? current.map((night) => night.id === editingNightId ? localNight : night) : [localNight, ...current]);
      ...
      const persistedMovies: Array<{ id: string; position: number; title: string; picker_member_id: string | null; picker_name: string | null; tmdb_id: number | null; poster_path: string | null; release_year: number | null }> = [];
      for (const movie of preparedMovies) {
        const movieData = { movie_night_id: night.id, position: movie.position, title: movie.title, picker_member_id: movie.pickerId, picker_name: movie.pickerName, tmdb_id: movie.tmdbId, poster_path: movie.posterPath, release_year: movie.releaseYear };
        const movieResult = movie.id
          ? await supabase.from("movie_night_movies").update(movieData).eq("id", movie.id).eq("movie_night_id", night.id).select("id, position, title, picker_member_id, picker_name, tmdb_id, poster_path, release_year").single()
          : await supabase.from("movie_night_movies").insert(movieData).select("id, position, title, picker_member_id, picker_name, tmdb_id, poster_path, release_year").single();
      ...
      const ratingRows = preparedMovies.flatMap((movie) => {
        ...
        return persistedMovie ? movie.ratings.map((rating) => ({ movie_id: persistedMovie.id, member_id: rating.memberId, member_name: rating.memberName, rating: rating.rating })) : [];
      });
      ...
      const savedNight = movieNightFromRows(night, persistedMovies, (ratingResult.data ?? []) as Array<{ id: string; movie_id: string; member_id: string | null; member_name: string; rating: number }>);
      setNights((current) => editingNightId ? current.map((item) => item.id === editingNightId ? savedNight : item).sort((first, second) => second.watchedOn.localeCompare(first.watchedOn)) : [savedNight, ...current].sort((first, second) => second.watchedOn.localeCompare(first.watchedOn)));
      notify(editingNightId ? "Movie night updated." : "Movie night saved.", "success");
      resetForm();
    } catch (error) {
