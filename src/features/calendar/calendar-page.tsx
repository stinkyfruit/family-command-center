...
export function CalendarPreview({ anchor, events, members, onOpenCalendar, onOpenDay }: { anchor: Date; events: Event[]; members: Member[]; onOpenCalendar: () => void; onOpenDay: (day: Date) => void }) {
  return <section className="hidden rounded-[1.75rem] bg-white p-5 shadow-sm ring-1 ring-slate-100 dark:bg-white/5 dark:ring-white/10 md:block md:p-7"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-violet-600">CALENDAR</p><h2 className="text-xl font-bold">This month</h2></div><button onClick={onOpenCalendar} className="rounded-xl bg-violet-600 px-4 py-2 text-sm font-bold text-white hover:bg-violet-700">Open calendar</button></div><div className="mt-4"><MonthGrid anchor={anchor} events={events} members={members} onOpenDay={onOpenDay} /></div></section>;
}
