...
import { CalendarPersonFilter, DayCalendar, MobileCalendarGrid, MonthGrid, WeekCalendar } from "@/components/home/calendar";
...
export const CalendarPage = memo(function CalendarPage({
  ...
  return <section className="rounded-[1.75rem] bg-white p-5 shadow-sm ring-1 ring-slate-100 dark:bg-white/5 dark:ring-white/10 md:p-7">
    ...
    <div className="md:hidden"><MobileCalendarGrid key={`${view}-${anchor.toISOString()}`} anchor={anchor} events={events} members={members} view={view} onEdit={onEditEvent} onAddEvent={onShowEventForm} onAnchorChange={onAnchorChange} /></div>
    <div className="hidden md:block">{view === "Day" ? <DayCalendar date={anchor} events={events} members={members} onEdit={onEditEvent} /> : view === "Week" ? <WeekCalendar anchor={anchor} events={events} members={members} onEdit={onEditEvent} onOpenDay={onOpenDay} /> : <MonthGrid anchor={anchor} events={events} members={members} onOpenDay={onOpenDay} />}</div>
