    ...
    <main className={dark ? "dark min-h-screen" : "min-h-screen"}>
      <div className="min-h-screen bg-[#f8f7ff] text-slate-900 transition-colors dark:bg-[#151522] dark:text-slate-100 lg:pl-24">
        <aside className="fixed inset-x-0 bottom-0 z-30 flex justify-around border-t border-slate-100 bg-white/95 p-2 shadow-lg backdrop-blur dark:border-white/10 dark:bg-[#1c1c2b]/95 lg:inset-y-0 left-0 right-auto w-24 flex-col justify-start gap-3 border-r border-t-0 px-3 py-6">
          <button onClick={() => setActiveTab("calendar")} title="Home" className="hidden lg:grid size-12 place-items-center self-center rounded-2xl bg-violet-600 text-white shadow-lg shadow-violet-300/50"><AppIcon name="home" className="size-6"/></button>
          <nav className="flex flex-1 justify-around gap-1 lg:mt-8 lg:flex-col lg:justify-start">{([ ["calendar", "calendar", "Calendar"], ["tasks", "tasks", "Tasks"], ["chores", "chores", "Chores"], ["lists", "lists", "Lists"], ["settings", "settings", "Settings"] ] as const).map(([tab, icon, label]) => <button key={tab} onClick={() => setActiveTab(tab)} title={label} className={`flex flex-col items-center gap-1 rounded-2xl px-3 py-2 text-xs font-bold transition-colors ${activeTab === tab ? "bg-violet-600 text-white shadow-md" : "text-slate-500 hover:bg-violet-50 dark:text-slate-300 dark:hover:bg-white/10"}`}><AppIcon name={icon} className="size-5"/><span className="hidden lg:block">{label}</span></button>)}</nav>
        </aside>
