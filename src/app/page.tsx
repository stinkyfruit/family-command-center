...
function MonthGrid({ anchor, events, members, onOpenDay }: { anchor: Date; events: Event[]; members: Member[]; onOpenDay: (date: Date) => void }) {
  ...
  return <div><div className="mb-1 grid grid-cols-7 gap-1">{weekdays.map((day) => <p key={day} className="p-1 text-center text-xs font-bold text-slate-400">{day}</p>)}</div><div className="grid grid-cols-7 gap-1">{days.map((day) => { const dayEvents = events.filter((event) => eventOccursOn(event, day)); const currentMonth = day.getMonth() === anchor.getMonth(); return <button key={day.toISOString()} onClick={() => onOpenDay(day)} className={`relative flex aspect-square min-h-0 flex-col items-stretch overflow-hidden rounded-xl p-2 text-left ${currentMonth ? "bg-slate-50 dark:bg-white/5" : "bg-slate-50/40 text-slate-300 dark:bg-white/[.02]"}`}><span className="flex h-7 shrink-0 items-start justify-start text-left text-sm font-bold leading-none">{day.getDate()}</span><span className="block min-h-0 space-y-1 overflow-hidden text-left">{dayEvents.slice(0, 2).map((event) => <EventChip key={event.id} event={event} members={members} compact />)}{dayEvents.length > 2 && <span className="block px-1 text-xs font-bold text-violet-600">+{dayEvents.length - 2} more</span>}</span></button>; })}</div></div>;
}
...
function TasksPage({ todos, members, onAdd, onToggle }: { todos: Todo[]; members: Member[]; onAdd: () => void; onToggle: (id: string | number) => void }) {
  ...
  return <section className="mx-auto max-w-[1600px] px-5 pb-8 md:px-9"><div className="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-100 dark:bg-white/5 dark:ring-white/10 md:p-8"><div className="flex items-center justify-between"><div><p className="text-sm font-bold text-rose-500">FAMILY TASKS</p><h2 className="text-3xl font-bold">Today&apos;s to-dos</h2></div><button onClick={onAdd} className="rounded-xl bg-rose-500 px-4 py-3 font-bold text-white">+ Add task</button></div><div className="mt-7 grid gap-4 md:grid-cols-2">{open.map((todo) => { const person = assignee(todo); return <button key={todo.id} onClick={() => onToggle(todo.id)} className="flex items-start gap-4 rounded-2xl bg-rose-50 p-5 text-left hover:bg-rose-100 dark:bg-rose-400/10 dark:hover:bg-rose-400/25"><span className="grid size-6 shrink-0 place-items-center rounded-full border-2 border-rose-400 text-rose-500">✓</span><span className="min-w-0 flex-1"><b className="block">{todo.title}</b><small className="mt-1 block text-slate-500">{todo.due || "No deadline"}</small>{person && <span className="mt-3 inline-flex rounded-full px-2.5 py-1 text-xs font-bold" style={{ backgroundColor: `${person.color ?? "#fda4af"}33`, color: person.color ?? "#be123c" }}>For {person.name}</span>}</span></button>; })}{open.length === 0 && <p className="text-slate-400">You&apos;re all caught up.</p>}</div>{completed.length > 0 && <div className="mt-8 border-t border-slate-100 pt-5 dark:border-white/10"><h3 className="font-bold text-emerald-600">Completed recently</h3><p className="mt-1 text-sm text-slate-500">Completed tasks stay here for 7 days, then move out of sight.</p><div className="mt-3 grid gap-3 md:grid-cols-2">{completed.map((todo) => <button key={todo.id} onClick={() => onToggle(todo.id)} className="flex items-center gap-3 rounded-xl bg-emerald-50 p-3 text-left text-sm text-emerald-800 hover:bg-emerald-100 dark:bg-emerald-400/10 dark:text-emerald-200 dark:hover:bg-emerald-400/20"><span>✓</span><span className="line-through">{todo.title}</span><span className="ml-auto text-xs">Restore</span></button>)}</div></div>}</div></section>;
}
...
function SettingsPage({ themeMode, onThemeModeChange, googleConnections, appleFeeds, onConnect, onToggleConnection, onAddApple, onToggleApple }: { themeMode: ThemeMode; onThemeModeChange: (mode: ThemeMode) => void; googleConnections: GoogleConnection[]; appleFeeds: AppleFeed[]; onConnect: () => void; onToggleConnection: (connection: GoogleConnection) => void; onAddApple: (name: string, url: string) => void; onToggleApple: (feed: AppleFeed) => void }) {
...
function ListCard({ list, colorIndex, onAddItem, onToggleItem, onDeleteList }: { list: SharedList; colorIndex: number; onAddItem: (listId: string | number) => void; onToggleItem: (listId: string | number, itemId: string | number) => void; onDeleteList: (list: SharedList) => void }) {
  const colors = ["bg-rose-100 dark:bg-rose-500/25", "bg-sky-100 dark:bg-sky-500/25", "bg-amber-100 dark:bg-amber-400/25", "bg-emerald-100 dark:bg-emerald-500/25", "bg-violet-100 dark:bg-violet-500/25", "bg-orange-100 dark:bg-orange-500/25"];
