...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState<"calendar" | "tasks" | "chores" | "lists">("calendar");
    ...
    <main className={dark ? "dark min-h-screen" : "min-h-screen"}>
      <div className="min-h-screen bg-[#f8f7ff] text-slate-900 transition-colors dark:bg-[#151522] dark:text-slate-100">
        <header className="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-4 px-5 py-5 md:px-9">
          ...
          <nav className="order-3 flex w-full gap-1 rounded-2xl bg-white p-1 shadow-sm ring-1 ring-slate-100 dark:bg-white/5 dark:ring-white/10 md:order-none md:w-auto">{([ ["calendar", "▦ Calendar"], ["tasks", "✓ Tasks"], ["chores", "✦ Chores"], ["lists", "☰ Lists"] ] as const).map(([tab, label]) => <button key={tab} onClick={() => setActiveTab(tab)} className={`rounded-xl px-4 py-2 text-sm font-bold ${activeTab === tab ? "bg-violet-600 text-white" : "text-slate-500 hover:bg-violet-50 dark:text-slate-300 dark:hover:bg-white/10"}`}>{label}</button>)}</nav>
        ...
        {activeTab === "calendar" ? <div className="mx-auto grid max-w-[1600px] gap-5 px-5 pb-8 md:px-9 xl:grid-cols-[minmax(0,2.6fr)_minmax(300px,1fr)]">
          <section className="space-y-5 xl:col-start-2 xl:row-start-1">
          ...
          <section className="rounded-[1.75rem] bg-white p-5 shadow-sm ring-1 ring-slate-100 dark:bg-white/5 dark:ring-white/10 md:p-7 xl:col-start-1 xl:row-span-2">
