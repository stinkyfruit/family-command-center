...
function WeekCalendar({ anchor, events, onSelect }: { anchor: Date; events: Event[]; onSelect: (date: Date) => void }) {
  ...
  return <div className="grid grid-cols-7 gap-1">{days.map((day) => { const dayEvents = events.filter((event) => sameDate(new Date(event.startsAt), day)); const isToday = sameDate(day, new Date()); return <button key={day.toISOString()} onClick={() => onSelect(day)} className={`aspect-square min-h-0 overflow-hidden rounded-xl p-2 text-left transition-colors ${isToday ? "bg-violet-600 text-white" : "bg-slate-50 hover:bg-violet-50 dark:bg-white/5 dark:hover:bg-white/10"}`}><div className="h-11"><p className={`text-xs font-bold ${isToday ? "text-white/75" : "text-slate-400"}`}>{day.toLocaleDateString([], { weekday: "short" })}</p><p className="text-lg font-bold">{day.getDate()}</p></div><div className="h-[calc(100%-2.75rem)] space-y-1 overflow-y-auto pr-0.5">{dayEvents.map((event) => <EventChip key={event.id} event={event} />)}</div></button>; })}</div>;
}
...
function MonthGrid({ anchor, events, onSelect }: { anchor: Date; events: Event[]; onSelect: (date: Date) => void }) {
  ...
  return <div><div className="mb-1 grid grid-cols-7 gap-1">{weekdays.map((day) => <p key={day} className="p-1 text-center text-xs font-bold text-slate-400">{day}</p>)}</div><div className="grid grid-cols-7 gap-1">{days.map((day) => { const dayEvents = events.filter((event) => sameDate(new Date(event.startsAt), day)); const currentMonth = day.getMonth() === anchor.getMonth(); return <button key={day.toISOString()} onClick={() => onSelect(day)} className={`aspect-square min-h-0 overflow-hidden rounded-xl p-2 text-left ${currentMonth ? "bg-slate-50 dark:bg-white/5" : "bg-slate-50/40 text-slate-300 dark:bg-white/[.02]"}`}><p className="h-6 text-sm font-bold">{day.getDate()}</p><div className="max-h-[calc(100%-1.5rem)] space-y-1 overflow-hidden">{dayEvents.slice(0, 2).map((event) => <EventChip key={event.id} event={event} />)}</div></button>; })}</div></div>;
}
