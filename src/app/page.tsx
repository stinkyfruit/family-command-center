...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from("todos").select("id, title, due_at, status, assignee_member_id").eq("household_id", householdId).neq("status", "archived").order("due_at"),
      ...
      if (todoResult.data) setTodos(todoResult.data.map((todo) => ({ id: todo.id, title: todo.title, due: todo.due_at ? new Date(todo.due_at).toLocaleDateString([], { weekday: "short" }) : "", done: todo.status === "completed", assigneeMemberId: todo.assignee_member_id })));
