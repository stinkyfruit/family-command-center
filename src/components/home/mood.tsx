...
export function FamilyMoodCard({
  ...
  function openMoodModal(member: Member) {
    ...
    {members.length === 0 ? <p className="mt-5 rounded-2xl bg-white/60 p-4 text-sm font-semibold text-slate-600 dark:bg-white/10 dark:text-slate-300">Add family members to start checking in.</p> : <>
      <div className="mt-4 grid grid-cols-2 gap-2">
        {members.map((member) => {
          ...
          return <button key={member.id} type="button" onClick={() => openMoodModal(member)} aria-haspopup="dialog" aria-label={`Update ${member.name}'s mood`} style={mood ? { backgroundColor: memberColor } : undefined} className={`flex min-w-0 cursor-pointer items-center gap-2 rounded-2xl p-2 text-left transition hover:brightness-105 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-600 ${mood ? "shadow-sm" : "bg-white/65 dark:bg-white/10"}`}>
