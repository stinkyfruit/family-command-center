...
export function parseIcal(text: string): ParsedEvent[] {
  ...
  for (const line of lines) {
    ...
    if (["UID", "SUMMARY", "DTSTART", "DTEND", "LOCATION", "DESCRIPTION"].includes(key)) current[key] = line.slice(divider + 1);
  }
  return events;
}
