...
- **UI consistency:** Match the existing visual language, spacing, interaction patterns, and responsive behavior. Reuse shared components and styles. Use animation when it improves feedback or adds appropriate seasonal atmosphere, keep it purposeful, and honor reduced-motion preferences. Use the shared Phosphor icon system through `AppIcon` for interface icons instead of introducing ad hoc Unicode symbols or a second icon library.
