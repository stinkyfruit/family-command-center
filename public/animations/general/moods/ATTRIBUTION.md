# Mood animations

These five animated Noto Emoji Lottie JSON files are from Google’s Noto Emoji
Animation project and are licensed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/):

- `1f600.json` — 😀 grinning face
- `1f642.json` — 🙂 slightly smiling face
- `1f610.json` — 😐 neutral face
- `1f634.json` — 😴 sleeping face
- `1f622.json` — 😢 crying face

Source: [quarrel/noto-emoji-dotlottie](https://github.com/quarrel/noto-emoji-dotlottie),
which packages the original animated JSON files from Google’s
[Noto Emoji Animation project](https://googlefonts.github.io/noto-emoji-animation/).

The additional mood animations (`1f929.json`, `1f60c.json`, `1f624.json`,
`1f61f.json`, `1f612.json`, `1f621.json`, `1f915.json`, `1f60b.json`,
`1f633.json`, `1f914.json`, `1f628.json`, `1f922.json`, `1f92a.json`,
`1f62c.json`, and `1f630.json`) are also Noto Emoji animations from the same
source:

- `1f612.json` — 😒 unamused face
- `1f621.json` — 😡 pouting face
- `1f915.json` — 🤕 face with head-bandage
- `1f60b.json` — 😋 face savoring food
- `1f633.json` — 😳 flushed face
- `1f914.json` — 🤔 thinking face
- `1f628.json` — 😨 fearful face
- `1f922.json` — 🤢 nauseated face (used for the custom “Bean Butt” mood)
- `1f92a.json` — 🤪 zany face
- `1f62c.json` — 😬 grimacing face
- `1f630.json` — 😰 anxious face with sweat
- `2708.json` — ✈️ airplane (used for the “Wanderlust” mood)

The “Fine” mood reuses the neutral-face animation in `1f610.json`.
